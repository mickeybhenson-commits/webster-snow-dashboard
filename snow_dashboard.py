...
with tab_radar:
    ...
    st.markdown("### 📡 Live Doppler Radar - High Resolution Multi-Scale View")
    
    # Enhanced controls
    col_mode, col_product = st.columns([2, 2])
    
    with col_mode:
        radar_type = st.radio(
            "Display Mode:",
            ["Animated Loop", "Static Snapshot", "Both"],
            horizontal=True,
            help="Animated shows movement, Static shows maximum detail"
        )
    ...
    with col_product:
        radar_product = st.selectbox(
            ...
            [
                ...
                "Precipitation Type (Dual-Pol)",
                "Storm Total Precipitation",
                "Base Velocity (Wind)"
            ],
            help="Select different radar products for specialized analysis"
        )
    
    st.markdown("---")
    
    # Map product selection to URL codes
    product_map = {
        "Base Reflectivity (Standard)": "",  # default, no suffix
        "Precipitation Type (Dual-Pol)": "_HC",  # Hydrometeor Classification
        "Storm Total Precipitation": "_STP",  # Storm Total Precip
        "Base Velocity (Wind)": "_BV"  # Base Velocity
    }
    ...
    if radar_type in ["Animated Loop", "Both"]:
        st.markdown("**Animated Loop:**")
        if product_suffix:
            # For specialized products, use static with note
    ...
    if "Precipitation Type" in radar_product:
        st.info("🧊 **Dual-Pol Technology**: Distinguishes rain, snow, ice, and mixed precipitation. Blue/cyan = snow/ice, Green = rain, Mixed colors = wintry mix")
    elif "Storm Total" in radar_product:
        st.info("📊 **Storm Total**: Shows accumulated precipitation since storm began. Useful for tracking total snowfall/rainfall amounts.")
    elif "Velocity" in radar_product:
        st.info("💨 **Base Velocity**: Red = moving away from radar, Green = moving toward radar. Useful for detecting rotation and wind patterns.")
    ...
    <div style="text-align: center; margin: 10px 0;">
        <a href="https://radar.weather.gov/station/kgsp/standard" target="_blank" style="color: #4ECDC4; font-size: 16px; font-weight: bold;">
            🔍 View Full Screen Interactive KGSP Radar →
        </a>
    </div>
    """, unsafe_allow_html=True)
    ...
    # Lightning Data
    with st.expander("⚡ Lightning Activity (Last Hour)"):
        st.markdown("""
        ### Real-Time Lightning Detection
        
        Lightning data from NOAA's GOES-16 Geostationary Lightning Mapper (GLM)
        """)
        
        # Lightning map (if available)
        st.markdown("""
        **Lightning Activity Map:**
        - 🟡 Yellow: 1-5 strikes in past hour
        - 🟠 Orange: 6-15 strikes
        - 🔴 Red: 15+ strikes
        
        **Winter Lightning:**
        - Rare but possible during heavy snow
        - Called "thundersnow"
        - Indicates very intense snowfall rates
        - Usually produces 2-4 inches per hour
        
        For live lightning data, visit: [LightningMaps.org](https://www.lightningmaps.org)
        """)
    ...
    with rad_col1:
        ...
        <div style="text-align: center; margin: 5px 0;">
            <a href="https://radar.weather.gov/region/southeast/standard" target="_blank" style="color: #4ECDC4; font-size: 14px;">
                🔍 Interactive SE Radar →
            </a>
        </div>
        """, unsafe_allow_html=True)
    ...
    with rad_col2:
        ...
        st.markdown("""
        <div style="text-align: center; margin: 5px 0;">
            <a href="https://radar.weather.gov/region/conus/standard" target="_blank" style="color: #4ECDC4; font-size: 14px;">
